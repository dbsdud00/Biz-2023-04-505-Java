 ...
 *  	=> 이 상황이 발생하면 사전에 팀원들 간의 업무 조율이 명확하게 이루어지고
 *  		모든 팀원이 공감한다면 상관 없다.
 *  		실제 상황에서는 이러한 문제가 발생하면 프로젝트 전체에 영향을 미치게 된다.
 *  		단, 계속해서 이 프로젝트에서 insertStudent() mehtod 를 계속 사용할 예정이라면 이 방법이 최선이다.
 *  		하지만, 어떤 특정 개발자의 필요에 의해서 InsertStudent() method가 작성되었다면, 절대 해서는 안될 일이다.
 *  2. 객체의 형 변환을 통하여 생성된 객체를 자신의 ImplV1 클래스로 형 변환하여 임시로 method를 사용하는 방법
 *  
 *  
 */
