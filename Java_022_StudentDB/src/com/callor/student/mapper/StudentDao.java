 ...
 *   = Mapper(SQL 문과 DTO 를 자동으로 매칭 시킨다.)
 *   
 * 	DB와 연결하여 CRUD 를 처리하는 객체
 * 
 *  MyBatis 를 사용하게 되면 Dao interface 를 생성하고
 *  각 method 에 Annotation 을 부착하여 SQL 명령문을 설정한다.
 *  
 *  그러면 나머지 여러가지 코드(DBMS 에서 데이터를 가져와서 DTO 에 담고, 다시 List 에 add 하여
 *  	Return 하는 이러한 코드를 Mybatic 의 SqlSessionFactory 가 대신 생성해 준다.
 *  
 */
