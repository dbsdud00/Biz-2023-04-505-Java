...
public class ExecF {
	public static void main(String[] args) {
		// 배열의 크기와 index 값을 알고 있을 때,
		// 또는 배열의 일부 요소만 사용하고 싶을 때
...
//		}
		 ...
		 *  						\ size()가 0 인 상태로 시작한다.
		 *  						\
		 *  개수 범위 내의 index	\ 필요에 따라 List에 add()를 실행하여 
		 *  위치 요소에 값 저장		\ 데이터를 추가한다.
		 *  str[3] = "Korea"		\
		 *  						\
		 *  개수 범위 내의 index	\ List의 get(index) method 를 통하여
		 *  위치 요소로 부터		\ 데이터를 읽을 수 있다
		 *  값을 읽을 수 있다.		\
		 *  println(str[0])			\ 특정 index 요소의 값을 변경할 때는
		 *  						\ List의 set(index) method를 통하여 
		 *   						\ 값을 변경할 수 있다
		 *   						\
		 *  배열은 코드가 실행되는 	\ List 는 필요에 따라 
		 *  도중에 크기를 변경 불가	\ size()가 변경될 수 있다
		 *  						\
